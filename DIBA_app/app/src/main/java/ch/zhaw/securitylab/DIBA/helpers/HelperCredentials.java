...
public class HelperCredentials
{
	...
	public void attemptTask()
	{
		...
		{
			password.setError(DIBA.get().getString(R.string.error_invalid_password));
			focusView = password;
			cancel = true;
		}
		...
		{
			email.setError(DIBA.get().getString(R.string.error_field_required));
			focusView = email;
			cancel = true;
		}
		else if (!isEmailValid(emailText))
		{
			email.setError(DIBA.get().getString(R.string.error_invalid_email));
			focusView = email;
			cancel = true;
		}
