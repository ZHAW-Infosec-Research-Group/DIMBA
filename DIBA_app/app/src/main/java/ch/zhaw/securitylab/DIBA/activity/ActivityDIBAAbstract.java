...
import ch.zhaw.securitylab.DIBA.helpers.Buttons;
import ch.zhaw.securitylab.DIBA.helpers.Toasty;
import ch.zhaw.securitylab.DIBA.navigation.InNavigation;
import ch.zhaw.securitylab.DIBA.navigation.NavigationAuth;
import ch.zhaw.securitylab.DIBA.navigation.NavigationUnAuth;

/**
 * The parent activity to all activities in InBank. It offers automatic assignment of toolbars and navigation drawers,
 * easy button creation and option menu support through reflection.
 */
public abstract class ActivityDIBAAbstract extends AppCompatActivity {
	// -------------------------------------------- //
	// FIELDS
	// -------------------------------------------- //
	...
	public ActivityDIBAAbstract(int viewId, ToolbarMode toolbarMode)
	{
		this(viewId, toolbarMode, 0);
	}
	
	public ActivityDIBAAbstract(int viewId, ToolbarMode toolbarMode, int toolbarFocus) {
		this(viewId, toolbarMode, toolbarFocus, true);
	}
	
	public ActivityDIBAAbstract(int viewId, ToolbarMode toolbarMode, int toolbarFocus, boolean screenshotSecure) {
		this.viewId = viewId;
	...
	private void initToolbar() {
		...
		// Security Fallback
		if (DIBA.get().getJwt() == null) navigation = new NavigationUnAuth(this);
	...
	private void sayLoggedInAs(Toolbar toolbar) {
		String name = DIBA.get().getUserName();
		if (name != null) {
	...
	private boolean isMethodOverriden(String method) {
		...
		try {
			if (method.equals("bug") && clazz.getName().equals("ch.zhaw.securitylab.DIBA.activity.auth.ActivityAuthMain")) {
				return true;
			}
