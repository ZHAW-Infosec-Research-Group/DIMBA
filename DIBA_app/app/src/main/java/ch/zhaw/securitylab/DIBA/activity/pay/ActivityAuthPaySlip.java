 ...
 */
public class ActivityAuthPaySlip extends ActivityDIBAAbstract {
	// -------------------------------------------- //
	// CONSTANTS
	// -------------------------------------------- //
	...
	private OnClickListener onPayslipReadDefault()
	{
		return (View v) ->
		{
			InputStream stream = DIBA.get().getResources().openRawResource(R.raw.payslip_default);
	...
	private boolean readFile(BufferedReader br) {
		...
		DIBA.get().setPaySlip(paySlipText);
